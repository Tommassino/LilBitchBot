...
class Module(object):
	def __init__(self, wrapper):
		...
		self.audio_lengths = {}
		self.messageHooks = {
			Command(re.compile('^!join .*'), JoinVoice('!join',self), "join", "Usage: !join <channel name>\n\tUse this command to make the bot join a voice channel. Example: !join General"),
			Command(re.compile('^!play .*'), PlaySound('!play',self, wrapper, 0.2), "play", "Usage: !play <sound name>\n\tEntertain your friend by playing a cool sound. For a list of available sounds see !list. Example !play titanic"),
			Command(re.compile('^!list'), ListVoice('!play',self), "list", "Usage: !list\n\tList the available sounds to play."),
			Command(re.compile('^!cost (.*)'), ListDuration('!cost',self), "cost", "Usage: !cost <sound name>\n\tCheck how much lil bitch points it will cost to play this sound.")
		}
...
class ListDuration(object):
	...
	def __call__(self,msg,mtc,cli):  
		yield from cli.send_message(msg.channel,"{0} lil bitches".format(self.module.poll_length(msg.content[len(self.cmd):].strip())*0.2))
