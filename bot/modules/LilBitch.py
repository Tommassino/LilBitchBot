...
class Module(object):
	def __init__(self, wrapper):
		...
		self.messageHooks = {
			Command(re.compile('.*lil.*bitch.*', flags=re.IGNORECASE), UserIncrement(self.bitchCounter)),
			Command(re.compile('.*who.*lil.*bitch.*', flags=re.IGNORECASE), ReplyTop(self.bitchCounter)),
			Command(re.compile('^!top$'), ListTop(self.bitchCounter,3), "top", "Usage: !top\n\tDisplay a list of the best lil bitches!"),
			Command(re.compile('^am i a lil bitch.*$', flags=re.IGNORECASE), ListBitch(self.bitchCounter)),
			Command(re.compile('^!drawTop$', flags=re.IGNORECASE), DrawTop(wrapper.money,3,wrapper), "drawTop", "Usage: !drawTop\n\tDisplay the best lil bitches in style!"),
			Command(re.compile('^!give (.*) (.*)'), Grant(wrapper.money, self), "give", "Usage: !give <username> <points>\n\tGive or subtract lil bitch points, admin-only")
		}
		self.lastSpendTime = {}
